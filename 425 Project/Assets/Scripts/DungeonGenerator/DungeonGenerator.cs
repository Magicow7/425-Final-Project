...
public class DungeonGenerator : SingletonMonoBehaviour<DungeonGenerator>
{
    ...
    private static int _dungeonRoomCurrID;
    ...
    [FormerlySerializedAs("roomsToSpawn"), Header("Settings for Static Generation"), SerializeField]
    ...
    private float _availableDoorheight;

    [FormerlySerializedAs("forceLevels"), SerializeField]
    private bool _forceLevels;
    ...
    [FormerlySerializedAs("maxGraphDistanceFromCenter"), Header("Settings for Dynamic Generation"), SerializeField]
    ...
    [FormerlySerializedAs("enemySpawner"), Header("Settings for Enemies"), SerializeField]
    private EnemySpawner _enemySpawner;


    [FormerlySerializedAs("visualizeBoundingBoxes"), Header("Settings for Debugging"), SerializeField]
    // visualize bounding boxes for debuging
    private bool _visualizeBoundingBoxes;

    //debug object
    [FormerlySerializedAs("testObject"), SerializeField]
    private GameObject _testObject;

    [FormerlySerializedAs("randomSeed"), Header("General Settings"), SerializeField]
    ...
    private List<DungeonRoomParameters> _possibleRoomParameters = new();

    private readonly List<DungeonDoorway> _doorways = new();
    ...
    private int _roomsAddedInLevel;
    private float _verticalRoomsOnCurrentLevel;
    ...
    private void Start()
    {
        ...
        {
            _random = new Random();
            _seed = _random.Next(0, int.MaxValue);
        }
    ...
    private void Update()
    {
        ...
        {
            var newCenterRoom = _currentCenterRoom;
    ...
    private void GenerateDynamicDungeon(DungeonRoom newCenterRoom)
    {
        ...
        var destroyQueue = new List<DungeonRoom>();
    ...
    private void DestroyRooms(List<DungeonRoom> roomsToDestroy)
    {
        ...
        {
            //remove prefabs
            foreach (var prefab in room.spawnedPrefabs)
            {
                Destroy(prefab);
            }
    ...
    private void SpawnRoomObjects()
    {
        ...
        foreach (var room in _rooms)
        {
            ...
            else
            {
                ...
                {
                    var boundingBoxVisualization = Instantiate(_testObject, room.worldPosition, room.worldRotation);
                    room.spawnedPrefabs.Add(boundingBoxVisualization.gameObject);
                    var pointA = room.parameters._boundingBoxMaxPoint + room.worldPosition;
                    var pointB = room.parameters._boundingBoxMinPoint + room.worldPosition;
                    var size = new Vector3(Mathf.Abs(pointA.x - pointB.x),
                        Mathf.Abs(pointA.y - pointB.y),
                        Mathf.Abs(pointA.z - pointB.z));
                    var center = room.worldPosition + new Vector3(0, (room.parameters._boundingBoxMaxPoint.y - room.parameters._boundingBoxMinPoint.y) / 2, 0);
                    ;
                    boundingBoxVisualization.transform.position = center;
                    boundingBoxVisualization.transform.localScale = size;

                    var worldTransformVisualization = Instantiate(_testObject, room.worldPosition, room.worldRotation);
                }

                //spawn in room object and block unsued doors
                var roomObject = Instantiate(room.parameters._roomObject, room.worldPosition, room.worldRotation);
                room.spawnedPrefabs.Add(roomObject.gameObject);
                foreach (var obj in room.parameters._roomObject.GetComponentsInChildren<DungeonRoomContainedObject>())
                {
            ...
            foreach (var door in room.doorways)
            {
                if (door.connectingRooms.Count == 2)
                {
                    ...
                    if (door.spawnedOpenDoorway == null)
                    {
                        ...
                        var openDoor = Instantiate(_doorwayOpen, door.worldPosition + new Vector3(0, 1, 0), room.worldRotation * Quaternion.Euler(door.parameters._angle));
                    ...
                    {
                        var link = _navMeshLinkHolder.AddComponent<NavMeshLink>();
                        var posDistanceFromDoor = GetOutVectorFromAngle(door.parentRoom.worldRotation.eulerAngles + door.parameters._angle);
                ...
                else
                {
                    ...
                    if (door.spawnedBlocker == null)
                    {
                        ...
                        var blocker = Instantiate(_doorwayBlocker, door.worldPosition + new Vector3(0, 1, 0), room.worldRotation * Quaternion.Euler(door.parameters._angle + new Vector3(0, 180, 0)));
    ...
    {
        var successfullyAddedRoom = false;
        //first room
        if (_rooms.Count == 0)
        {
            //initialize start room
            var newRoom = InitializeStartRoom(_startRoom, Vector3.zero);
            //add to roomlist
            _rooms.Add(newRoom);
            //add doorways to doorwaylist
            for (var j = 0; j < newRoom.doorways.Count; j++)
            {
        ...
        else
        {
            ...
            while (orderedDoorsToTry.Count > 0 && !successfullyAddedRoom)
            {
                ...
                var choosenDoorway = orderedDoorsToTry.Dequeue();
                ...
                {
                    //dequeue top of list
                    var choosenParams = orderedTemplatesToTry.Dequeue();
                    // get list of door indicies to try to add at on room template
                    var doorIndiciesToTry = new List<int>();
                    for (var i = 0; i < choosenParams._relativeDoorParameters.Count; i++)
                    {
                    ...
                    while (orderedIndiciesToTry.Count > 0 && !successfullyAddedRoom)
                    {
                        ...
                        var newRoom = result.dungeonRoom;
    ...
    private DungeonRoom InitializeStartRoom(DungeonRoomParameters dungeonRoomParameters, Vector3 startPosition)
    {
        ...
        for (var i = 0; i < dungeonRoomParameters._relativeDoorParameters.Count; i++)
        {
            var newDoorway = new DungeonDoorway(dungeonRoomParameters._relativeDoorParameters[i], newRoom.worldPosition + dungeonRoomParameters._relativeDoorParameters[i]._relativePosition, newRoom);
    ...
    {
        var successfullyAddedRoom = true;
        DungeonRoom newRoom;
        //get the angle of the door in world space, stored in the y value
        var fromDoorOutVector = GetOutVectorFromAngle(branchFromDoor.parentRoom.worldRotation.eulerAngles + branchFromDoor.parameters._angle);
        ...
        var modifiedParameters = result.returnedParams;
        var appliedRotation = result.returnedQuaternion;
        ...
        var newPosition = branchFromDoor.worldPosition;
        ...
        for (var i = 0; i < modifiedParameters._relativeDoorParameters.Count; i++)
        {
            var newDoorPosition = newRoom.worldPosition + modifiedParameters._relativeDoorParameters[i]._relativePosition;
            var newDoorway = new DungeonDoorway(modifiedParameters._relativeDoorParameters[i], newDoorPosition, newRoom);
            newRoom.doorways.Add(newDoorway);
        }

        //check validity of bounding boxes
        var roomsToCheck = new List<DungeonRoom>(_rooms);
    ...
    {
        var returnParameters = inParams;
        //Get outward direction of connecting door
        var currentAttatchingDoorOutVector = GetOutVectorFromAngle(inParams._relativeDoorParameters[attachingDoorIndex]._angle);
        //get reverse of the branching door's outward direction, this is what we want out connection door to point towards.
        var targetVector = -vectorToOppose;
        //calulate target up (this is overkill now, it was going to be used if doors could face in angles other than only y rotation)
        var targetUp = prevRotation * Quaternion.Euler(inParams._relativeDoorParameters[attachingDoorIndex]._angle) * Vector3.up;
        //get quaternion rotation needed to point room towards target vector
        var rotationToApply = GetAlignmentQuaternion(currentAttatchingDoorOutVector, targetVector);
        //rotate all door parameters on room parameter by rotationToApply.
        var newDoorParameters = new List<DungeonDoorParameters>();
        for (var i = 0; i < returnParameters._relativeDoorParameters.Count; i++)
        {
            var rotatedPoint = RotateAround(returnParameters._relativeDoorParameters[i]._relativePosition, inParams._centerPoint, rotationToApply);
    ...
    private Quaternion GetAlignmentQuaternion(Vector3 currentForward, Vector3 targetForward)
    {
        ...
        var rotationNeeded = Quaternion.Euler(new Vector3(0, angleY, 0));
    ...
    private Vector3 RotateAround(Vector3 point1, Vector3 point2, Quaternion rotation)
    {
        ...
        var translatedPoint = point1 - point2;
        ...
        var finalPoint = rotatedTranslatedPoint + point2;
    ...
    {
        var returnList = new Queue<T>();
        //loop through all posibilities
        while (possibilities.Count > 0)
        {
            var choosenOne = possibilities[0];
            var totalWeight = 0f;
            //get total weight of all possibilities
            foreach (var elem in possibilities)
            {
    ...
    {
        var returnList = new List<DungeonDoorway>();
    ...
    private List<DungeonRoom> GetDungeonRoomNeighbors(DungeonRoom checking)
    {
        var returnList = new List<DungeonRoom>();
        foreach (var doorway in checking.doorways)
        {
            foreach (var room in doorway.connectingRooms)
            {
    ...
    private void UpdateNeighborCenterDistances(DungeonRoom currRoom, DungeonRoom prevRoom)
    {
        //case of center room
    ...
    private bool AreObBsIntersecting(Obb obb1, Obb obb2)
    {
        Vector3[] axes1 = obb1.axes;
        Vector3[] axes2 = obb2.axes;

        // The 15 potential separating axes
        var separatingAxes = new List<Vector3>();
        ...
        for (var i = 0; i < 3; i++)
        {
            for (var j = 0; j < 3; j++)
            {
                var crossProduct = Vector3.Cross(axes1[i], axes2[j]);
    ...
    //OBB helper function
    private bool IsOverlappingOnAxis(Obb obb1, Obb obb2, Vector3 axis)
    {
    ...
    //OBB helper function
    private float GetProjectionLength(Obb obb, Vector3 axis)
    {
        // Project the half-extents of the OBB onto the axis
        float projectionLength = 0;

        for (var i = 0; i < 3; i++) // Loop over the OBB's 3 axes
        {
    ...
    //parameter structs   
    [Serializable]
    private struct DungeonRoomParameters
    {
        ...
        [FormerlySerializedAs("boundingBoxMinPoint")]
        public Vector3 _boundingBoxMinPoint;

        [FormerlySerializedAs("boundingBoxMaxPoint")]
        public Vector3 _boundingBoxMaxPoint;
        ...
        [FormerlySerializedAs("chestPosition")]
        public Vector3 _chestPosition;

        [FormerlySerializedAs("chestRotation")]
        public Vector3 _chestRotation;

        //list of door parameters for doors on this room
        [FormerlySerializedAs("relativeDoorParameters")]
        public List<DungeonDoorParameters> _relativeDoorParameters;
    }

    [Serializable]
    private struct DungeonDoorParameters
    {
    ...
    private class DungeonRoom
    {
        ...
        public readonly List<DungeonRoomContainedObject> dungeonRoomObjects = new();

        //base parameters of room
        public readonly DungeonRoomParameters parameters;
        public readonly int roomId;

        public readonly List<GameObject> spawnedPrefabs = new();
        ...
        {
            foreach (var roomObject in dungeonRoomObjects)
            {
    ...
    private struct Obb
    {
        ...
        {
            var localCenter = Vector3.Lerp(roomParams._boundingBoxMaxPoint, roomParams._boundingBoxMinPoint, 0.5f);
