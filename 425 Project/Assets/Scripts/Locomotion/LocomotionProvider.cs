...
namespace Locomotion
{
    public abstract class LocomotionProvider : MonoBehaviour
    {
        ...
        protected void SetVelocityXZ(Vector2 velocity)
        {
            _setVelocityXZ(velocity);
        }

        protected void SetVelocityY(float velocity)
        {
            _setVelocityY(velocity);
        }

        protected void SetVelocityXZ(Vector3 velocity)
        {
        ...
        protected void SetVelocityY(Vector3 velocity)
        {
        ...
        protected void AddVelocityXZ(Vector2 velocity)
        {
            SetVelocityXZ(locomotionManager.VelocityXZ + velocity);
        }

        protected void AddVelocityXZ(Vector3 velocity)
        {
            SetVelocityXZ(locomotionManager.VelocityXZ + new Vector2(velocity.x, velocity.z));
        }

        protected void AddVelocityY(float velocity)
        {
            SetVelocityY(locomotionManager.VelocityY + velocity);
        }

        protected void AddVelocityY(Vector3 velocity)
        {
            SetVelocityY(locomotionManager.VelocityY + velocity.y);
        }
        ...
        protected void AddRotation(float rotation)
        {
            SetRotation(locomotionManager.Rotation + rotation);
        }

        protected void SetMovementState(MovementState state, bool on)
        {
            _setMovementState(state, on);
        }
